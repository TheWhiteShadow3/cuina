...
public class ResizableFrame extends CuinaWidget
{
	...
	public void addTrigger(Trigger trigger)
	{
		if (closeButton == null)
		{
			...
			closeButton.getModel().addListener(new Listener()
			{
				@Override
				public void handleEvent(WidgetEvent event)
				{
					if (event.type == WidgetEvent.ACTION_PERFORMED)
						testTriggers(FRAME_CLOSED, null);
				}
			});
			add(closeButton);
			layoutCloseButton();
	...
	{
		boolean result = super.removeTrigger(trigger);
		if (closeButton != null)
		{
			closeButton.setVisible(getTriggerCount() > 0);
		}
		return result;
	}
...
// " ms");
	...
	protected void layoutTitle()
	{
		...
		if (titleAreaLeft != titleAreaRight && titleAreaTop != titleAreaBottom)
		{
			if (titleWidget == null)
			{
				titleWidget = new TextWidget(getAnimationState(), false);
	...
	protected void layoutCloseButton()
	{
		if (closeButton != null)
		{
			...
			closeButton.setPosition(getTitleX(closeButtonX), getTitleY(closeButtonY));
			closeButton.setVisible(getTriggerCount() > 0 && hasCloseButton);
		}
	}
	...
	protected void layoutResizeHandle()
	{
		...
		if (resizeHandle != null)
		{
			...
			resizeHandle.setPosition(getTitleX(resizeHandleX), getTitleY(resizeHandleY));
			resizeHandle.setVisible(hasResizeHandle && resizableAxis == ResizableAxis.BOTH);
		}
		else
		{
...
//		{
//			resizeHandle.getAnimationState().setAnimationState(TextWidget.STATE_HOVER, false);
//		}
//
...
//		if (!isMouseExit && resizeHandle != null && resizeHandle.isVisible())
//		{
//			resizeHandle.getAnimationState().setAnimationState(TextWidget.STATE_HOVER, resizeHandle.isMouseInside(evt));
//		}
//
//		if (!evt.isMouseDragEvent())
//		{
//			if (evt.getType() == Event.Type.MOUSE_BTNDOWN && evt.getMouseButton() == Event.MOUSE_LBUTTON
//					&& handleMouseDown(evt)) { return true; }
//		}
//
...
//	}
	...
	{
		if (resizeHandle != null && resizeHandle.isVisible() && !isMouseInside())
		{
			resizeHandle.getAnimationState().setAnimationState(TextWidget.STATE_HOVER, false);
		}
		...
		if (resizeHandle != null && resizeHandle.isVisible() && isMouseInside())
		{
			resizeHandle.getAnimationState().setAnimationState(
					TextWidget.STATE_HOVER, resizeHandle.isInside(Input.mouseX(), Input.mouseY()));
		}
		
		if (Input.isPressed("mouse.left") && handleMouseDown()) return;
	}
	...
	private DragMode getDragMode(int mx, int my)
	{
		...
		if (closeButton != null && closeButton.isVisible() && closeButton.isInside(mx, my)) { return DragMode.NONE; }

		if (resizableAxis == ResizableAxis.NONE) { return DragMode.NONE; }

		if (resizeHandle != null && resizeHandle.isVisible() && resizeHandle.isInside(mx, my)) { return resizeHandleDragMode; }
		...
		if (left)
		{
			if (top) { return DragMode.CORNER_TL; }
			if (bot) { return DragMode.CORNER_BL; }
			return DragMode.EDGE_LEFT;
		}
		if (right)
		{
			if (top) { return DragMode.CORNER_TR; }
			if (bot) { return DragMode.CORNER_BR; }
			return DragMode.EDGE_RIGHT;
		}
		if (top) { return DragMode.EDGE_TOP; }
		if (bot) { return DragMode.EDGE_BOTTOM; }
		return DragMode.NONE;
	}

	private boolean handleMouseDown()
	{
	...
	{
		final int dx = Input.mouseX() - dragStartX;
		final int dy = Input.mouseY() - dragStartY;
	...
	private void setArea(int top, int left, int right, int bottom)
	{
		...
		setSize(Math.max(getMinWidth(), right - left), Math.max(getMinHeight(), bottom - top));
	}
}
