...
//			areas.clear();
		...
		File file = new File(Game.getRootPath() + File.separator + "maps", key + ".cxm");
			...
			for(int y = 0; y < map.height; y++)
			{
				...
				for(int z = 2; z >= 0; z--)
				{
					...
					if (id > 0 && id < Tileset.AUTOTILES_OFFSET)
					{
						...
						{ // TODO: Implementiere Priorität und Passage für Autotiles
							if (tileset.getPriorities()[id] == 0) break;
						}
					}
				}
...
//		MapObject obj;
		for(Integer key : map.objects.keySet())
		{
			...
			NOTE: Wegen Änderungen an der Objekt-Datenklasse nicht mehr gültig
...
//				System.out.println("Teste: " + xx + ", " + yy);
	 ...
	 * @param x X-Position in Felder
	 * @param y Y-Position in Felder
	 */
...
//	}
	...
	{
//		System.out.println("lösche Objekt (" + obj.getID() + ") " + obj.getName() + ": " + obj.getX() + ", " + obj.getY());
