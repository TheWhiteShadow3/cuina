...
public class PluginBlock
{
    ...
    public void setDefaults(ILaunchConfigurationWorkingCopy config)
    {
        ...
        pluginDir = new File(enginePath, pluginPath);
        config.setAttribute(CuinaLaunch.PLUGIN_PATH, pluginPath);
        config.setAttribute(CuinaLaunch.PLUGIN_ENTRIES, plugins);
    }
    
    public void initializeFrom(ILaunchConfiguration config) throws CoreException
    {
    	enginePath = config.getAttribute(CuinaLaunch.ENGINE_PATH, "");
        pluginDir = new File(config.getAttribute(CuinaLaunch.PLUGIN_PATH, ""));
        plugins = config.getAttribute(CuinaLaunch.PLUGIN_ENTRIES, new HashSet<String>());
        inPlugin.setText(pluginDir.toString());
    ...
    private void findPlugins(File dir)
    {
        ...
        if (originalConfig != null) try
        {
            ...
            {
                plugins = originalConfig.getAttribute(CuinaLaunch.PLUGIN_ENTRIES, new HashSet<String>());
                pluginTable.setCheckedElements(plugins.toArray());
                return;
            }
        }
    ...
    private class PluginListener implements ModifyListener, SelectionListener, ICheckStateListener
    {
        ...
        public void modifyText(ModifyEvent e)
        {
        	...
        	if (!path.isAbsolute())
        		pluginDir = new File(enginePath + '/' + inPlugin.getText());
        	else
        		pluginDir = path.toFile();
